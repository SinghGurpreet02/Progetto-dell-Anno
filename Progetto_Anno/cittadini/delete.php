...
require_once("../inc/tokenvalID_cittadiniator.inc.php"); // OPZIONALE per rispondere solo ad utenti autenticati con token

$database = new Database();
$db = $database->getConnection();

// Il TokenValID_cittadiniator è necessario solo quando il servizio risponde ad utenti autenticati 
if($database->GetAuthenticated() && !Tokenvalidator($conn,$user)){
	http_response_code(401);
	exit;
}

if(isset($_GET['ID_cittadini'])) // Recupero l'ID_cittadini dalla querystring
{
	$cittadini = new Cittadini($db);
	$cittadini->ID_cittadini = $_GET['ID_cittadini'];

	if($cittadini->delete()){
		http_response_code(200); // OK (https://it.wikipedia.org/wiki/Codici_di_stato_HTTP)
		$rspMsg = new Responsemessage("cittadini eliminato correttamente", $cittadini->ID_cittadini); 
	} else {
		http_response_code(503); //503 service unavailable
		$rspMsg = new Responsemessage("Impossibile eliminare il cittadini", -1); 
	}
} else {
	http_response_code(503); //503 service unavailable
	$rspMsg = new Responsemessage("ID_cittadini non indicato", -1); 
}
